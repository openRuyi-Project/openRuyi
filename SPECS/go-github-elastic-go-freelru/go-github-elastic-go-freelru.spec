...
# The noarch build still runs tests on the worker CPU. The benchmark helper has
# no riscv64 assembly implementation, so keep only the portable library tests.
%if "%{_host_cpu}" == "riscv64"
%define go_test_exclude_glob  %{go_import_path}/bench
%endif
