...
%install
# Install the upstream tree, then remove development-only files and Python
# bytecode so the noarch runtime package contains only reproducible inputs.
...
# Check the installed entry points and the two RISC-V integration markers used
# to select the correct K3s asset and openEuler host variables at runtime.
