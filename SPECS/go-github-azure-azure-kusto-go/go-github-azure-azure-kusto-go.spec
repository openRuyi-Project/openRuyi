...
# Preserve the sibling module directories under the shared repository path.
%install
install -d "%{buildroot}%{go_sys_gopath}/%{go_import_path}"
cp -a ./. "%{buildroot}%{go_sys_gopath}/%{go_import_path}/"

# Test each module from one GOPATH tree so sibling imports resolve locally.
# Compile excluded packages without running their environment-dependent tests.
%check
export GO111MODULE=off
# Select Go 1.23 synchronous timer behavior explicitly for GOPATH test binaries.
