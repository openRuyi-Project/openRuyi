...
# Keep vet enabled for the callback test.
Patch2000:      2000-report-callback-test-error-without-formatting.patch

# fvt_client_test.go requires an external MQTT broker on ports 1883 and 8883.
# File-store, memory-store, packet and other unit tests remain enabled.
BuildOption(check):  -skip '^Test_(Start|Disconnect|Publish_(1|2|3|BytesBuffer)|Subscribe|Will|CleanSession|Binary_Will|p[012]s[012]|PublishMessage|PublishEmptyMessage|CallbackOverrun|MultipleURLs|ping1_idle5|autoreconnect|cleanUpMids(_2)?|ConnectRetry(Publish)?|ResumeSubs(WithReconnect|MaxInflight)?|DisconnectWhileProcessingIncomingPublish)$'
