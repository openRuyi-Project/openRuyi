...
# These tests rely on GOPATH-era test binary paths, call os.Exit from the test
# process, or assert legacy crypto behavior and error text that changed in Go
# 1.26. Keep the source packages installed while excluding only those tests.
...
# Vet rejects unkeyed StringPop test literals and unreachable strftime code.
...
# Retain compilation coverage for tests whose execution is excluded above.
%check -a
