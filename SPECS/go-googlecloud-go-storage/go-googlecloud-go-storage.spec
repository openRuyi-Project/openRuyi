...
Source0:        https://github.com/googleapis/google-cloud-go/archive/refs/tags/storage/v1.62.2.tar.gz#/%{_name}-%{version}.tar.gz
BuildArch:      noarch
BuildSystem:    golangmodules

BuildOption(prep):  -n google-cloud-go-storage-v1.62.2
...
BuildRequires:  go(cloud.google.com/go/internal/optional)
BuildRequires:  go(cloud.google.com/go/internal/testutil)
BuildRequires:  go(cloud.google.com/go/internal/trace)
BuildRequires:  go(cloud.google.com/go/internal/uid)
BuildRequires:  go(cloud.google.com/go/internal/version)
...
Provides:       go(cloud.google.com/go/storage/control/apiv2) = %{version}
Provides:       go(cloud.google.com/go/storage/control/apiv2/controlpb) = %{version}
Provides:       go(cloud.google.com/go/storage/dataflux) = %{version}
Provides:       go(cloud.google.com/go/storage/experimental) = %{version}
Provides:       go(cloud.google.com/go/storage/internal) = %{version}
Provides:       go(cloud.google.com/go/storage/internal/apiv2) = %{version}
Provides:       go(cloud.google.com/go/storage/internal/apiv2/storagepb) = %{version}
Provides:       go(cloud.google.com/go/storage/internal/test/conformance) = %{version}
Provides:       go(cloud.google.com/go/storage/transfermanager) = %{version}
...
Requires:       go(cloud.google.com/go/internal/optional)
Requires:       go(cloud.google.com/go/internal/testutil)
Requires:       go(cloud.google.com/go/internal/trace)
Requires:       go(cloud.google.com/go/internal/uid)
Requires:       go(cloud.google.com/go/internal/version)
...
# grpc_dp_diag_test.go requires a GCE DirectPath/auth environment; OBS reports
# "directPathDiagnostic() = not_on_gce". grpc_metrics_test.go creates a Google
# Cloud metrics exporter and fails without ADC credentials.
rm -f grpc_dp_diag_test.go grpc_metrics_test.go
# TestAppendableWriteUnsupported calls newHTTPStorageClient(), which fails in
# OBS with "google: could not find default credentials"; keep the other tests in
# http_client_test.go by deleting only that credential-dependent test function.
sed -i '/^func TestAppendableWriteUnsupported/,/^func TestValidateChecksumFromServer/{/^func TestValidateChecksumFromServer/!d;}' http_client_test.go
...
# no recorded replay data or Google Cloud credentials are available.
go test -v -short ${_go_filtered}
popd
