...
%check
# WMI has no Linux buildable files. Build the Windows tests for amd64 because
# Go does not support the windows/riscv64 target inherited on riscv64 workers.
%go_common
cd %{_builddir}/go/src/%{go_import_path}
GOOS=windows GOARCH=amd64 %__go test -c -o %{_builddir}/wmi.test.exe %{go_import_path}
