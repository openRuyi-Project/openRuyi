...
%ifarch riscv64
# The fixture's 250 ms channel expiry races snapshot I/O on OBS workers.
%define nats_skip_snapshot ^TestClusteringFollowerDeleteOldChannelPriorToSnapshotRestore$
%endif
...
# Pass upstream's SQL flag after the package path; OBS has no database service.
# Run packages separately so long-running server tests stream their progress.
for test_package in $(%__go list ./...); do
    %__go test %{shrink:%{go_test_flags_default}} -timeout 20m \
        %{?nats_skip_snapshot:-skip '%{nats_skip_snapshot}'} "$test_package" -sql=false
done
%{buildroot}%{_bindir}/%{_name} -v
